...
- Personal Markdown — memory files, skills, rules, Obsidian notes — relaxes line length (`rumdl check -d MD013`): these files use single long prose lines and leave wrapping to the viewer. The relaxations ride the command because `rumdl` discovers config files only from the working directory, never the checked file's ancestors. Two kinds relax more:
  - Memory topic files and `MEMORY.md` (`-d MD013,MD041`): they open with YAML frontmatter and carry no H1 by design.
  - Obsidian notes (`-d MD013,MD025,MD032`): vault notes use H1 section headings and lists flush under their headings by design.
- Repo Markdown runs bare `rumdl check`, leaving each repo's own config in charge.
