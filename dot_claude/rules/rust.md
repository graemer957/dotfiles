...
- The enum sibling: code that classifies or decides per variant of a same-crate enum (`needs_x`, `can_y`, kind→value maps) is an exhaustive `match` with no `_` arm, written as a method on the enum — never a predicate chain (`is_a() || is_b()`) at a call site — so a new variant is a compile error that forces a decision. When the surrounding file's idiom *is* the predicate chain, that's a justified divergence to flag, not a pattern to copy.
...
- When authoring `macro_rules!` arms, end each matcher with `$(,)?` so rustfmt's trailing comma doesn't fail token-matching; and treat any arm no call site expands as unproven — expansion is the only type check a macro body gets, so an uncalled macro compiles clean regardless of what's inside it.
- A collection's length belongs in its type only when the length is the invariant. Declare shared constants and sequence parameters as `&[T]`, not `[T; N]` — the array form makes adding an element edit the type as well as the value, for a guarantee nobody relies on. Keep `[T; N]` where `N` is load-bearing: an exhaustive enumeration (`VALUES: [Self; 3]`), a fixed-size buffer, a hash width. Expect one use-site cost: a slice iterates to `&T`, so the loop binding destructures (`for &x in XS`) where an array yielded `T` by value.
